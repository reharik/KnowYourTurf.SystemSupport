...
if (typeof dci.mediaHandler== "undefined") {
            dci.mediaHandler = {};
}

dci.mediaHandler.controller = (function(){
    var setupEvents = function(result,mediaType,availableItems,selectedItems){
        ...
        $(result).find("#"+mediaType+"Input").tokenInput(availableItems?availableItems:{},{prePopulate: selectedItems,
            internalTokenMarkup:function(item){
                var cssClass = tooltipAjaxUrl ? "class='dci_tokenTooltip selectedItem' rel='"+tooltipAjaxUrl+"?EntityId="+item.id +"'":"class='selectedItem'";
                return "<p><a "+cssClass+">"+ item.name +"</a></p>";
            },
            afterTokenSelectedFunction:function(){ $(".dci_tokenTooltip").cluetip({showTitle: false,
