...
(function($) {
	...
	plugin.execute = function(settings, zindex){
        ...
        // objects
        var wrapper = $("<div/>").attr("id",self.attr("id")+"wrapper");
        var overlay = $("<div/>").attr("id",self.attr("id")+"overlay");
        var input = $("<input/>").attr("id",self.attr("id")+"input");
        var selector = $("<select/>").attr("id",self.attr("id")+"selector");
...
//                "background-color": "red"
            ...
            var ousidew = self.outerWidth();
            self.css("width", "auto");
            var ow = ow > self.outerWidth() ? ow : self.outerWidth();
            self.css("width", ousidew);
        ...
        function enable(e, s, v) {
            ...
            if(self.find("option:first").text() && self.find("option:first").text().length>0)
            {
                self.prepend("<option />");
            }
    ...
    function register(nsp) {
    	...
    	$.fn[nsp] = function(settings) {
            ...
            if(settings=="reset"){
                ...
                $(this).unwrap();
                $("div#"+$(this).attr("id")+"overlay").remove();
                $("input#"+$(this).attr("id")+"input").remove();
                $("select#"+$(this).attr("id")+"selector").remove();
                $(this).width(width);
